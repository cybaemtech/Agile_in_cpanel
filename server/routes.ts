...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/projects/:projectId/team-members', canAccessProject, async (req, res) => {
    try {
  ...
  // Admin: Can create all types (EPIC, FEATURE, STORY, TASK, BUG)
  // Scrum Master: Can create EPIC, FEATURE  
  // Member: Can create STORY, TASK, BUG
  app.post('/api/work-items', canManageWorkItemType([]), async (req, res) => {
    try {
      ...
      // Validate if assignee exists and is a team member of the project (if assigneeId is provided)
      if (workItemData.assigneeId) {
        ...
        // Check if assignee is a team member of the project
        if (project.teamId) {
          const teamMembers = await storage.getTeamMembers(project.teamId);
          const isTeamMember = teamMembers.some(member => member.userId === workItemData.assigneeId);
          if (!isTeamMember) {
            return res.status(403).json({ message: "Assignee must be a team member of the project" });
          }
        }
      }
      ...
      // Ensure externalId is set (will be generated by storage if not provided)
      const workItemToCreate = {
        ...workItemData,
        externalId: workItemData.externalId || '' // Storage will generate if empty
      };
      
      const workItem = await storage.createWorkItem(workItemToCreate);
  ...
  // Get all work items across all projects (for calendar and global views)
  ...
  app.get('/api/projects/:projectId/work-items', canAccessProject, async (req, res) => {
    try {
      const projectId = parseInt(req.params.projectId);
  ...
  app.patch('/api/work-items/:id', canManageWorkItemType(['EPIC', 'FEATURE', 'STORY', 'TASK', 'BUG']), async (req, res) => {
    try {
      const workItemId = parseInt(req.params.id);
      ...
      // Validate assignee is a team member of the project (if assigneeId is being updated)
      if (validUpdates.assigneeId) {
        const assignee = await storage.getUser(validUpdates.assigneeId);
        if (!assignee) {
          return res.status(404).json({ message: "Assignee not found" });
        }
        
        // Get the project to check team membership
        const project = await storage.getProject(existingWorkItem.projectId);
        if (!project) {
          return res.status(404).json({ message: "Project not found" });
        }
        ...
        if (project.teamId) {
          const teamMembers = await storage.getTeamMembers(project.teamId);
          const isTeamMember = teamMembers.some(member => member.userId === validUpdates.assigneeId);
          if (!isTeamMember) {
            return res.status(403).json({ message: "Assignee must be a team member of the project" });
          }
        }
  ...
  app.get("/api/projects/:id/statistics", canAccessProject, async (req, res) => {
    try {
      const projectId = parseInt(req.params.id);
      if (isNaN(projectId)) {
      ...
      const assigneeStats = [];
      for (const [assigneeId, count] of Array.from(assigneesMap.entries())) {
