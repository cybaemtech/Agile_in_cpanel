 ...
 */
// Helper function to create child items - moved outside main function
async function createChildItems(
  ...
  for (let i = 0; i < itemCount && projectItemCountRef.count < maxItemsForProject; i++) {
    ...
    const uniqueId = `${shortTimestamp}${projectObj.id}${createdCountRef.count}${i}`;
    ...
    const item = await storage.createWorkItem({
      ...
      projectId: projectObj.id, // Fixed: use projectObj.id instead of parent.projectId
      parentId: parent.id,
    ...
    createdCountRef.count++;
    
    // If this is a feature or story, potentially add children
    if (type === "FEATURE" && Math.random() < 0.8) {
      await createChildItems(projectObj, item, "STORY", 1, 2, projectItemCountRef, maxItemsForProject, workItemTemplates, statuses, priorities, users, adminUser, uniqueTimestamp, createdCountRef);
    } else if (type === "STORY" && Math.random() < 0.7) {
      await createChildItems(projectObj, item, "TASK", 1, 2, projectItemCountRef, maxItemsForProject, workItemTemplates, statuses, priorities, users, adminUser, uniqueTimestamp, createdCountRef);
    }
  }
}
...
export async function generateRandomWorkItems(count: number = 150) {
  try {
    ...
    for (const project of projects) {
      ...
      if (epics.length === 0) {
        ...
        for (let i = 0; i < epicCount && projectItemCount < itemsForProject; i++) {
          ...
          const uniqueId = `${uniqueTimestamp}-${project.id}-${createdCountRef.count}`;
          ...
          await createChildItems(project, epic, "FEATURE", 2, 4, projectItemCountRef, itemsForProject, workItemTemplates, statuses, priorities, users, adminUser, uniqueTimestamp, createdCountRef);
        ...
        for (const epic of epics) {
          ...
          if (features.length === 0) {
            // Add new features
            const projectItemCountRef = { count: projectItemCount };
            await createChildItems(project, epic, "FEATURE", 2, 3, projectItemCountRef, itemsForProject, workItemTemplates, statuses, priorities, users, adminUser, uniqueTimestamp, createdCountRef);
            projectItemCount = projectItemCountRef.count;
          } else {
            ...
            for (const feature of features) {
              ...
              if (stories.length === 0) {
                // Add new stories
                const projectItemCountRef = { count: projectItemCount };
                await createChildItems(project, feature, "STORY", 2, 3, projectItemCountRef, itemsForProject, workItemTemplates, statuses, priorities, users, adminUser, uniqueTimestamp, createdCountRef);
                projectItemCount = projectItemCountRef.count;
              } else {
                ...
                for (const story of stories) {
                  ...
                  if (Math.random() < 0.7) {
                    // Add tasks
                    const projectItemCountRef = { count: projectItemCount };
                    await createChildItems(project, story, "TASK", 1, 2, projectItemCountRef, itemsForProject, workItemTemplates, statuses, priorities, users, adminUser, uniqueTimestamp, createdCountRef);
                    projectItemCount = projectItemCountRef.count;
                  } else {
                    // Add bugs
                    const projectItemCountRef = { count: projectItemCount };
                    await createChildItems(project, story, "BUG", 1, 1, projectItemCountRef, itemsForProject, workItemTemplates, statuses, priorities, users, adminUser, uniqueTimestamp, createdCountRef);
                    projectItemCount = projectItemCountRef.count;
                  }
                }
              }
    ...
    log(`Successfully generated ${createdCountRef.count} random work items`);
    return true;
