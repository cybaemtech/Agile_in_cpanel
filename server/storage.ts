...
export interface IStorage {
  ...
  // Analytics methods
  getWorkItemsCountByStatus(projectId?: number): Promise<Record<string, number>>;
  getWorkItemsCountByType(projectId?: number): Promise<Record<string, number>>;
  getWorkItemsCountByPriority(projectId?: number): Promise<Record<string, number>>;
}
...
export class MemStorage implements IStorage {
  ...
  async deleteTeam(id: number): Promise<boolean> {
    ...
    teamMembersToDelete.forEach(tm => {
  ...
  async getWorkItemsCountByStatus(projectId?: number): Promise<Record<string, number>> {
    const items = projectId 
      ? Array.from(this.workItems.values()).filter(item => item.projectId === projectId)
      : Array.from(this.workItems.values());
...
// Feature flag to use database storage vs in-memory storage
const useDatabase = process.env.USE_DB === 'true' || !!process.env.MYSQL_DATABASE_URL;

// Initialize storage
let storageInstance: IStorage | null = null;

// For now, use MemStorage to avoid PostgreSQL migration complexity
// Set USE_DB=true to enable DatabaseStorage once it's fixed for PostgreSQL
async function createStorage(): Promise<IStorage> {
  if (useDatabase) {
    console.log('🔄 Attempting to use database storage...');
    try {
      // Dynamic import to avoid eager evaluation of DatabaseStorage
